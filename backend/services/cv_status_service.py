...
class CVStatusUpdate:
    ...
    def __init__(self, stage: CVProcessingStage, message: str, progress: int = 0, details: str = ""):
        self.stage = stage
...
class CVStatusManager:
    ...
    def update_status(self, session_id: str, stage: CVProcessingStage, details: str = "") -> CVStatusUpdate:
        ...
        print(f"📊 [STATUS] {session_id}: {status_update.message} ({status_update.progress}%)")
        print(f"🔍 [STATUS] Stage: {stage.value}, Total sessions: {len(self._status_storage)}")
        if details:
    ...
    def get_current_status(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if session_id not in self._status_storage or not self._status_storage[session_id]:
            return None
    ...
    def cleanup_old_sessions(self, max_age_hours: int = 24):
        ...
        for session_id, status_list in self._status_storage.items():
            if status_list:
                last_update_time = datetime.fromisoformat(status_list[-1]["timestamp"]).timestamp()
...
def log_status_update(session_id: str, stage: CVProcessingStage, details: str = "") -> CVStatusUpdate:
...
def get_status_for_frontend(session_id: str) -> Dict[str, Any]:
    ...
    print(f"🗄️ [STATUS] Available sessions: {list(cv_status_manager._status_storage.keys())}")
    ...
    print(f"✅ [STATUS] Found status for {session_id}: {current_status['stage']} ({current_status['progress']}%)")
