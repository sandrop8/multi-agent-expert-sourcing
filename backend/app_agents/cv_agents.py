    ...
    try:
        ...
        # Check stored CV ID - get original filename from database
            ...
            try:
                ...
                with engine.connect() as conn:
                    result = conn.execute(
                        sa.select(cvs.c.original_filename).where(cvs.c.id == cv_id)
                    ).first()
                    ...
                    if result and result.original_filename:
                        ...
                        if "shaw" in original_filename:
                            cv_type = "shaw"
                            print(
                                f"🔍 [TESTING] Found Shaw CV: stored_cv_id:{cv_id} -> {result.original_filename}"
                            )
                        elif "nel" in original_filename:
                            cv_type = "nel"
                            print(
                                f"🔍 [TESTING] Found Nel CV: stored_cv_id:{cv_id} -> {result.original_filename}"
                            )
        ...
        try:
            if cv_type == "shaw":
                ...
                from data.ground_truths.janine_nel_ground_truth import (
                    extract_janine_nel_cv,
                )
        ...
        try:
            if isinstance(extraction_result, str):
                try:
                    ...
                    if "extracted_text" in extracted_data and isinstance(
                        extracted_data["extracted_text"], str
                    ):
                        ...
                        try:
                            ...
                            print(
                                f"⚠️ [TESTING] Could not parse inner JSON: {inner_text[:200]}..."
                            )
                            return

                except json.JSONDecodeError:
                    print(
                        f"⚠️ [TESTING] Extraction result is not JSON: {extraction_result[:200]}..."
                    )
                    return
            else:
        ...
        print("🎯 [TESTING] PERSONAL INFO FIELDS ONLY - Accuracy Benchmark")
        ...
        for field, description in personal_comparisons:
            ...
            if gt_value == ext_value:
                ...
                if isinstance(gt_value, str) and isinstance(ext_value, str):
                    if (
                        gt_value.lower() in ext_value.lower()
                        or ext_value.lower() in gt_value.lower()
                    ):
                    ...
                    common = set(str(x).lower() for x in gt_value) & set(
                        str(x).lower() for x in ext_value
                    )
                    if len(common) > 0:
                        print(
                            f"🟡 {description}: PARTIAL MATCH ({len(common)} common items)"
                        )
            ...
            comparison_results = {
                ...
                "field_comparisons": {
                    field: {
                        ...
                        "match_type": "exact"
                        if gt_personal.get(field) == ext_personal.get(field)
                        else "partial"
                        if gt_personal.get(field) and ext_personal.get(field)
                        else "missing",
                    }
                    for field, _ in personal_comparisons
                },
                "employment_count": {
                    "ground_truth": gt_emp_count,
                    "extracted": ext_emp_count,
                },
                "education_count": {
                    "ground_truth": gt_edu_count,
                    "extracted": ext_edu_count,
                },
            }
...
async def cv_validation_guardrail(ctx, agent, input_data):
    ...
    try:
        ...
        has_cv_indicators = any(
            indicator in input_str for indicator in obvious_cv_indicators
        )

        if has_cv_indicators:
            print(
                "🛡️ [GUARDRAIL] Found CV indicators in input - proceeding with validation"
            )

            # Run the guardrail validation agent
            print("🛡️ [GUARDRAIL] Running CV validation agent...")
            result = await Runner.run(
                cv_guardrail_agent, input_data, context=ctx.context
            )
            final_output = result.final_output_as(CVValidationResult)

            print(
                f"🛡️ [GUARDRAIL] Agent validation - Valid CV: {final_output.is_valid_cv}, Confidence: {final_output.confidence_score}"
            )
        else:
            print("🛡️ [GUARDRAIL] No CV indicators found - creating fallback validation")
        ...
        validation_passed = (
            final_output.is_valid_cv
            and final_output.confidence_score >= 0.6  # Lowered from 0.7
        )

        if validation_passed:
            print("✅ [GUARDRAIL] Validation PASSED - allowing processing")
        else:
            print("❌ [GUARDRAIL] Validation FAILED - blocking processing")
...
async def process_cv_workflow(file_path: str, session_id: str = None) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            print(
                f"📝 [WORKFLOW] Session context set for extraction tools: {session_id}"
            )
        except ImportError:
            print(
                "⚠️ [WORKFLOW] Could not set session context - status updates in tools may be limited"
            )
        ...
        print(
            f"🚀 [WORKFLOW] Starting CV processing workflow for: {file_path} (session: {session_id})"
        )
        ...
        print(
            f"🎯 [WORKFLOW] Calling Freelancer Profile Manager with input: {cv_input}"
        )
        print(
            f"📋 [WORKFLOW] Manager has {len(freelancer_profile_manager.handoffs)} handoff agents available"
        )
        print(
            f"🛠️ [WORKFLOW] Manager has {len(freelancer_profile_manager.tools)} tools available"
        )
        print(
            f"🛡️ [WORKFLOW] Manager has {len(freelancer_profile_manager.input_guardrails)} guardrails configured"
        )

        print("▶️ [WORKFLOW] Executing agent workflow...")
        ...
        log_status_update(
            session_id, CVProcessingStage.GAP_ANALYSIS, "Analyzing profile completeness"
        )

        # Status Update: Finalizing results
        log_status_update(
            session_id, CVProcessingStage.FINALIZING, "Preparing final analysis results"
        )
        print("✅ [WORKFLOW] CV processing workflow completed successfully")
        print(f"📤 [WORKFLOW] Final output type: {type(result.final_output)}")
        if result.final_output:
            output_preview = (
                str(result.final_output)[:200] + "..."
                if len(str(result.final_output)) > 200
        ...
        log_status_update(
            session_id, CVProcessingStage.COMPLETED, "Your CV analysis is ready!"
        )
        ...
        return {
            ...
            if result.final_output
            else "CV processed successfully",
        ...
        log_status_update(
            session_id, CVProcessingStage.ERROR, f"Processing failed: {str(e)}"
        )
