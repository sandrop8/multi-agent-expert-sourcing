...
def test_ground_truth_structure():
    ...
    try:
        ...
        print(f"  - Professional services fields: {len(ground_truth['professional_services'])}")
        print(f"  - Employment history: {len(ground_truth['employment_history'])} positions")
        print(f"  - Education: {len(ground_truth['education'])} degrees")
        print(f"  - Skills detailed: {len(ground_truth['skills_detailed'])} skills")
        print(f"  - Certifications: {len(ground_truth['certifications'])} certifications")
    ...
    try:
        ...
        mock_extracted = {
            "personal_info": {
                ...
                "skills_array": ["Business Development", "Recruitment", "Sales"],  # Partial match
            ...
            "professional_services": {
                "management_sales_hr_services": ["HR services", "sales services"],  # Partial match
            ...
            "skills_detailed": [
                {"skill_name": "Business Development", "skill_category": "business", "proficiency_level": "expert", "is_primary_skill": True},
                {"skill_name": "Recruitment", "skill_category": "business", "proficiency_level": "expert", "is_primary_skill": True}
            ],
            "certifications": [],
            "projects": []
        }
        ...
        partial_matches = sum(1 for comp in comparisons if 0.3 <= comp.match_score < 0.9)
        ...
        for i, comp in enumerate(comparisons[:10]):  # Show first 10
            status = "✅" if comp.match_score >= 0.9 else "🟡" if comp.match_score >= 0.3 else "❌"
            print(f"  {status} {comp.field_name}: {comp.match_score:.2f} - {comp.notes}")
...
def create_sample_cv_file():
    ...
    if not os.path.exists(lisa_cv_path):
        print(f"⚠️ Lisa.jpg not found. Creating a placeholder file for testing...")
...
""")
        ...
        print(f"📄 Created Lisa_placeholder.txt for testing")
...
async def main():
    ...
    try:
        ...
        if cv_file_path == "Lisa_placeholder.txt":
            print("⚠️ Using placeholder file - CV extraction will likely fail, but we can test the framework")
